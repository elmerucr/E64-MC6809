...
#define RAM_SIZE_CPU_VISIBLE 0x10000
...
void E64::blitter_ic::terminal_init(uint8_t number, uint8_t flags_0, uint8_t flags_1,
		   uint8_t size_in_pixels_log2, uint8_t tile_size_log2, uint16_t foreground_color,
		   uint16_t background_color)
{
	blit[number].background		= flags_0 & 0x02 ? true : false;
	blit[number].multicolor_mode	= flags_0 & 0x04 ? true : false;
	blit[number].color_per_tile	= flags_0 & 0x08 ? true : false;
	blit[number].use_cbm_font	= flags_0 & 0x80 ? true : false;

	blit[number].hor_stretch	= flags_1 & 0x01 ? true : false;
	blit[number].ver_stretch	= flags_1 & 0x04 ? true : false;
	blit[number].hor_flip		= flags_1 & 0x10 ? true : false;
	blit[number].ver_flip		= flags_1 & 0x20 ? true : false;

	blit[number].set_size_in_pixels_log2(size_in_pixels_log2);
	blit[number].set_tile_size_in_pixels_log2(tile_size_log2);
...
{
	for (size_t i=0; i < blit[number].tiles; i++) {
...
void E64::blitter_ic::terminal_putsymbol(uint8_t number, char symbol)
{
	...
	if (blit[number].cursor_position >= blit[number].tiles) {
		terminal_add_bottom_row(number);
		blit[number].cursor_position -= blit[number].columns;
	}
}
...
int E64::blitter_ic::terminal_putchar(uint8_t number, int character)
{
	...
	switch (result) {
		case '\r':
			blit[number].cursor_position -= blit[number].cursor_position % blit[number].columns;
			break;
		case '\n':
			blit[number].cursor_position -= blit[number].cursor_position % blit[number].columns;
			if ((blit[number].cursor_position / blit[number].columns) == (blit[number].rows - 1)) {
				terminal_add_bottom_row(number);
			} else {
				blit[number].cursor_position += blit[number].columns;
			}
			break;
		case '\t':
			while ((blit[number].cursor_position % blit[number].columns) & 0b11) {
...
void E64::blitter_ic::terminal_cursor_decrease(uint8_t no)
{
	...
	if (blit[no].cursor_position > blit[no].tiles - 1) {
		blit[no].cursor_position = blit[no].tiles - 1;
...
void E64::blitter_ic::terminal_cursor_right(uint8_t no)
{
	...
	if (blit[no].cursor_position > blit[no].tiles - 1) {
		...
		blit[no].cursor_position -= blit[no].columns;
	}
}
...
void E64::blitter_ic::terminal_cursor_up(uint8_t no)
{
	...
	if (blit[no].cursor_position >= blit[no].tiles) {
		uint32_t address;
...
{
	uint16_t start_of_line = blit[no].cursor_position - (blit[no].cursor_position % blit[no].columns);
	for (size_t i = 0; i < blit[no].columns; i++) {
		blit[no].command_buffer[i] = terminal_get_tile(no, start_of_line + i);
	}
	size_t i = blit[no].columns - 1;
...
{
	blit[no].cursor_position += blit[no].columns;
	for (int i=blit[no].tiles-1; i >= (blit[no].cursor_position - blit[no].get_current_column()) + blit[no].columns; i--) {
		terminal_set_tile(no, i, terminal_get_tile(no, i - blit[no].columns));
		terminal_set_tile_fg_color(no, i, terminal_get_tile_fg_color(no, i - blit[no].columns));
		terminal_set_tile_bg_color(no, i, terminal_get_tile_bg_color(no, i - blit[no].columns));
	}
	uint16_t start_pos = blit[no].cursor_position - blit[no].get_current_column();
	for (int i=0; i < blit[no].columns; i++) {
...
{
	uint16_t no_of_tiles_to_move = blit[no].tiles - blit[no].columns;
	...
	for (size_t i=no_of_tiles_to_move; i < blit[no].tiles; i++) {
...
enum E64::terminal_output_type E64::blitter_ic::terminal_check_output(uint8_t no, bool top_down, uint32_t *address)
{
	...
	for (int i = 0; i < blit[no].tiles; i += blit[no].columns) {
